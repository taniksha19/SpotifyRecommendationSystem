...
def get_streamings(path: str = r'C:\Users\HP\31119_TanikshaDatar\Projects\Spotify Recommendation\MyData', 
                ) -> List[dict]:
    
    '''Returns a list of streamings form spotify MyData dump.
    Will not acquire track features.'''
    
    files = [r'C:\Users\HP\31119_TanikshaDatar\Projects\Spotify Recommendation\MyData/' + x for x in listdir(path)
...
def get_saved_ids(tracks, path: str = r'C:\Users\HP\31119_TanikshaDatar\Projects\Spotify Recommendation\output/track_ids.csv') -> dict:
    track_ids = {track: None for track in tracks}
    folder, filename = path.split('/')
    if filename in listdir(folder):
        try:
            idd_dataframe = pd.read_csv(r'C:\Users\HP\31119_TanikshaDatar\Projects\Spotify Recommendation\output/track_ids.csv', 
                                     names = ['name', 'idd'])
...
def get_saved_features(tracks, path = r'C:\Users\HP\31119_TanikshaDatar\Projects\Spotify Recommendation\output/features.csv'):
